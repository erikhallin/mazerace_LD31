                   ...
                   int nCmdShow)
{
    ...
    g_mouse_but[0]=false; g_mouse_but[1]=false;
    g_mouse_but[2]=false; g_mouse_but[3]=false;
    g_mouse_pos[0]=0; g_mouse_pos[1]=0;

    //run game loop
    bool end_loop=false;
    game Game;
    int screen_size[2]={g_window_width,g_window_height};
    if( !Game.init(screen_size,g_keys,g_mouse_but,g_mouse_pos) )
    {
    ...
    while (!end_loop)
    {
        ...
        {
            //quit test
            if(g_keys[VK_ESCAPE]) end_loop=true;
